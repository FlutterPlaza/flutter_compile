...
- `fcp codepush release` (iOS) now keeps the app's dependency packages patch-compatible as well, not just the app's own libraries. The first release after upgrading recompiles once more than usual.
...
- `fcp codepush patch` now warns when the target iOS release was built without widget guarding or without the interface freeze; `--allow-unguarded-release` acknowledges and silences it. Releases created by older versions are not affected.
  - **Behavior change:** invalid `--rollout` values (for example `50%`, `fifty`, or an empty value — which previously defaulted to a silent full 100% rollout), unrecognized or empty `--platform` values (on both `release` and `patch` — any casing of the six documented values is now accepted and normalized, and `--platform android` with `--build` is rejected up front in favor of `apk`/`appbundle`), and an empty `--signing-key`, `--channel`, `--baseline`, `--patch-file`, `--package-prefix`, `--patch-entry-file`, or `--release-id` (on `patch`), an empty `--snapshot`, `--version`, `--app-id`, or `--baseline-id` (on `release`), or an empty `--flutter-version` (on both) are now rejected up front instead of being silently misread; argument mistakes are reported before any build work runs (a `--snapshot` path that doesn't exist is now itself reported up front: as an error when no `--build` could create it — previously a general failure later — and as a pre-build warning with `--build` for a path outside `build/` (a missing path under `build/` stays silent, since the build is about to create it); a `--snapshot` that names a directory — for example an app bundle instead of the binary file inside it — gets directory-specific guidance and is an up-front error even with `--build`, except for a directory under `build/` that the build itself can replace, which stays a warning — an `.app` bundle directory is an up-front error even there, since no build replaces a bundle directory with a file; a `--snapshot` that is a symbolic link whose target doesn't exist gets link-specific guidance — restore the target — as an up-front error without `--build`, while with `--build` it defers to the post-build check, since the build may create the link's target), and build-only flags passed without `--build` — and iOS-only flags on a non-iOS `--build` — now draw a warning on both commands instead of being silently ignored. When an upload will be signed, naming a pre-existing `--patch-file` now warns that signing rewrites that file in place (keep a copy if you need the original bytes). Release versions are validated up front on every platform (letters, digits, `.`, `_`, `+`, `-` only), and quoted pubspec `version:` values and trailing comments are now read correctly. Values of id-, version-, channel-, and package-URI-shaped flags are also trimmed of surrounding whitespace before use (a padded value previously mis-resolved silently). An iOS release uploaded via `--snapshot` now requires `--baseline-id` or `--allow-missing-baseline` regardless of how `--platform ios` is cased; and when `--build` is combined with a `--snapshot` that names bytes other than the build's own output, the saved baseline app and per-release archive are skipped (with an explanatory note), since they would not match what the release serves — and if those bytes carry no readable baseline identity and none is passed via `--baseline-id`, the release is rejected with guidance rather than created as one no device would ever update.
...
- **Behavior change:** `fcp codepush release` for iOS now requires a baseline identity. `--build` stamps one automatically; when releasing a pre-built app, pass `--baseline-id` (it is read from the built app automatically when available), or opt out with `--allow-missing-baseline`. Stamping now requires a writable `ios/Runner/` *directory*, not just a writable `Info.plist` (the stamp is written via a temp-and-rename so a failed write never truncates the plist). **Behavior change:** when `ios/Runner/Info.plist` — or Android's `assets/codepush.yaml` — is a symbolic link into shared configuration, or sits inside a linked directory (a shared `assets/` dir), the stamp-and-restore now writes through the link(s) and preserves them; previously the Android write replaced a linked file with a regular one. Two consequences for linked configs: during the build, the link's target briefly carries this release's values (visible to sibling projects reading the shared file), and if the post-build restore fails, it is the link's *target* that needs cleaning — the failure message names it, since `git checkout` of the link path cannot restore the target. In a checkout where that directory is read-only (some CI mounts and build sandboxes), either make it writable, commit an `FCPBaselineId` into the plist beforehand, or pass `--allow-missing-baseline`; when a `--build` release of that build's own output cannot stamp, `--baseline-id` is now accepted only if it exactly matches an id the just-built app already embeds — a flag the shipped bytes contradict (or that nothing embeds) is rejected instead of being recorded as an identity no device would ever present; with `--allow-missing-baseline` also set, the contradicting flag is dropped with a warning and the release proceeds under the id the built app embeds when it has one, or without an identity when it doesn't. **Behavior change:** when the stamp succeeds, the id the built app actually *embeds* is now authoritative over it — a differing embedded id is recorded instead, since that is the id devices present; and if the built app embeds none (typically a flavored `INFOPLIST_FILE` or a `GENERATE_INFOPLIST_FILE = YES` target that never packages `ios/Runner/Info.plist`), the release now fails after the build with guidance instead of being recorded under an id the shipped app doesn't carry — such a release could never be matched by a device. Point the target at `ios/Runner/Info.plist`, commit an `FCPBaselineId` into the plist the target actually uses, or pass `--allow-missing-baseline`; an explicit `--baseline-id` cannot substitute there, since devices only send an embedded id. Releases without an identity are never delivered to devices, so the command now fails fast instead of creating one silently. In projects with both `android/` and `ios/`, `fcp codepush release` now asks for `--platform` whenever neither `--platform` nor `--build` is given — including `--snapshot` releases, whose platform determines which checks apply.
...
- The code push app id is now recorded per project, so working on more than one code-push app on the same machine no longer repoints the others. `fcp codepush init` writes a `.flutter_compilerc` beside your `pubspec.yaml`; it holds only the app id — no credentials — so it is safe to commit. Projects that set none keep using `~/.flutter_compilerc` exactly as before, and `fcp config` says when a project overrides the key. The latest id is also mirrored into `~/.flutter_compilerc`, and `init` says so, so a project that has not been re-initialized still resolves an id. `fcp codepush init --app-id <existing>` records an existing app without creating a new one. Values in either file may be written as `key: value`; the surrounding spaces are ignored. Editing either file by hand now keeps your comments and blank lines, and comment lines are no longer listed as settings.
- `fcp codepush release --build` and `fcp codepush patch --build` now check your saved login before the build starts, so an expired session no longer costs a full build.
