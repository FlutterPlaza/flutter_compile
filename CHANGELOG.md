...
- iOS release builds are now prepared so patches can declare new `StatelessWidget` / `StatefulWidget` / `State` subclasses — for example, a patch that adds a new screen. The fix applies at release build time: apps already in the field need one fresh `fcp codepush release --build` to benefit.
